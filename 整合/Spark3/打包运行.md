...
### 建表+action
```bash
/opt/spark-3.1.2-bin-hadoop2.7/bin/spark-submit --class top.chendaye666.scala.ods.OdsNcddztTableSql \
        --master yarn \
        --name ods_table \
        ...
        --executor-memory 1g \
        --executor-cores 1 \
        /opt/work/datalake/sparkicebergddl-1.0-SNAPSHOT.jar

> 注意: `User did not initialize spark context!` -> 注释掉代码中 `.master()` 配置; 另外, 要使用spark3 运行 jar
> UI界面: `http://hadoop01:8080/`
...
/opt/flink-1.12.5/bin/flink run -m yarn-cluster -ynm ods -c top.chendaye666.v2.KafkaToSparkIcebergOds  -yjm 1024 -ytm 1024 -d /opt/work/datalake/ods-1.0-SNAPSHOT.jar
