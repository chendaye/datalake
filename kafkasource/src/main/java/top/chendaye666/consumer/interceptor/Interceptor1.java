package top.chendaye666.consumer.interceptor;
 ...
 * 消费完成后打印位移信息
 */
public class Interceptor1 implements ConsumerInterceptor<String, String> {
    ...
    public ConsumerRecords<String, String> onConsume(ConsumerRecords<String, String> consumerRecords) {
        ...
        for (TopicPartition partition : consumerRecords.partitions()){
            List<ConsumerRecord<String, String>> recordsInPartion = consumerRecords.records(partition);
            List<ConsumerRecord<String, String>> filterRecords = new ArrayList<>();

            for (ConsumerRecord<String, String> r : recordsInPartion){
                if (System.currentTimeMillis() - r.timestamp() < EXPIRE_INTERVAL){
                    filterRecords.add(r);
                }
                if (!filterRecords.isEmpty()){
                    newRecords.put(partition, filterRecords);
                }
            }
        }
